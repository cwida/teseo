...
Undo::Undo(TransactionImpl* tx, RollbackInterface* data_structure, uint32_t length) : m_transaction(tx), m_data_structure(data_structure), m_next(nullptr), m_length_payload(length), m_active(false) {
...
std::pair<Undo*, uint64_t> Undo::prune(Undo* head, const TransactionSequence* sequence){
    ...
    assert(context::thread_context()->epoch() && "Because this method involves a managed object of the GC (the argument `sequence'), it needs to invoked inside an epoch");
