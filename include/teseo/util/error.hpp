...
extern std::stringstream g_exception_stream; // helper to create the exception message
extern std::mutex g_exception_mutex; // mutual exclusion to create the exception message
...
#define RAISE_EXCEPTION_CREATE_ARGUMENTS(msg) const char* file = __FILE__; int line = __LINE__; const char* function = __FUNCTION__; \
        std::unique_lock<std::mutex> lock(::teseo::util::g_exception_mutex); \
        auto& stream = ::teseo::util::g_exception_stream; \
