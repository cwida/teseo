...
class RollbackInterface; // forward decl.
class TransactionImpl; // forward decl.
class TransactionSequence; // forward decl.

/**
 * Header associated to a single entry in the Undo Buffer
 */
class Undo {
    TransactionImpl* m_transaction; // transaction that performed the update
    RollbackInterface* m_data_structure; // pointer to the data structure when the update has been performed
    ...
    bool m_active; // whether the update is also in the data structure
...
public:
     ...
     */
    Undo(TransactionImpl* tx, RollbackInterface* data_structure, uint32_t length);
    ...
    // Check whether this undo record is active
    bool is_active() const;

    // Mark as active the current undo record
    void set_active(Undo* next);
